...
async function loadScores(){try{const r=await window.storage.get(LB_KEY);return r?JSON.parse(r.value):[];}catch(e){return[];}}
async function saveScore(e){try{const s=await loadScores();s.push(e);s.sort((a,b)=>b.score-a.score);const t=s.slice(0,10);await window.storage.set(LB_KEY,JSON.stringify(t));return t;}catch(e){return[];}}
...
// ─── CELLS ───────────────────────────────────────────────────────────────────
const CELLS={
  neutrophil:{name:"Neutrophil",cost:15,maxHp:60, dmg:7, range:95, aps:1.3, col:"#26C6DA",r:14,desc:"Fires at 3 targets at once. +25% vs bacteria.",multishot:3,catBonus:{bacteria:1.25}},
  macrophage:{name:"Macrophage",cost:30,maxHp:150,dmg:28,range:60, aps:0.55,col:"#66BB6A",r:21,desc:"Tank. High HP. +25% vs bacteria & fungi.",catBonus:{bacteria:1.25,fungi:1.25}},
...
const LEVEL_TIPS=[
  "Neutrophils fire at 3 targets at once — great for early swarms. Place them in a ring first.",
  ...
  "Parasite zigzag makes them unpredictable — place cells to intercept their path.",
...
function getMods(ch){const m={maxHp:100,eStart:60,eRegen:2.0,cMult:{},atkMult:1};if(ch.gender==="female")m.cMult.bcell=1.15;if(ch.gender==="male")m.cMult.tcell=1.15;if(ch.age==="child"){m.maxHp=80;m.eStart=85;m.eRegen=2.6;}if(ch.age==="young"){m.maxHp=100;m.eStart=60;m.eRegen=2.0;}if(ch.age==="adult"){m.maxHp=120;m.eStart=60;m.eRegen=1.8;}if(ch.age==="senior"){m.maxHp=75;m.eStart=75;m.eRegen=1.5;m.atkMult=0.82;}return m;}
function buildQ(lev,wave,now){const q=[],ws=1+(wave-1)*lev.waveScale;for(const[type,count]of lev.s){const spread=7000+wave*500;for(let i=0;i<count;i++)q.push({type,at:now+600+(i/Math.max(count-1,1))*spread+Math.random()*700,scale:ws});}return q.sort((a,b)=>a.at-b.at);}
function getBriefingEnemies(lev){const seen=[];for(const[type]of lev.s)if(!seen.find(e=>e.type===type))seen.push({type,...PATHS[type],...(FACTS[type]||{icon:"🦠",fact:""})});return seen;}

// ─── ENEMY DRAWING ────────────────────────────────────────────────────────────
function drawEnemy(ctx,p,now){
  const isCancer=p.cat==="cancer";
  const glowR=p.type==="tumor"?p.r*3.8:p.r*2.8;
  const pg=ctx.createRadialGradient(p.x,p.y,0,p.x,p.y,glowR);pg.addColorStop(0,p.col+(isCancer?"66":"44"));pg.addColorStop(1,"transparent");
  ctx.fillStyle=pg;ctx.beginPath();ctx.arc(p.x,p.y,glowR,0,Math.PI*2);ctx.fill();
  if(p.type==="tumor"){
    ctx.beginPath();for(let i=0;i<=24;i++){const a=i/24*Math.PI*2;const lump=p.r*(1+0.25*Math.sin(i*3+now/900)+0.12*Math.sin(i*5-now/650));const px=p.x+Math.cos(a)*lump,py=p.y+Math.sin(a)*lump;i===0?ctx.moveTo(px,py):ctx.lineTo(px,py);}
  ...
  if(p.cat==="virus"){
    ...
    for(let i=0;i<spk;i++){const base=i/spk*Math.PI*2;const wobble=0.18*Math.sin(now/400+i*2.1+p.id);const a=base+wobble;const tip=p.r+5+3*Math.abs(Math.sin(now/500+i*1.7));ctx.beginPath();ctx.moveTo(p.x+Math.cos(a)*p.r,p.y+Math.sin(a)*p.r);ctx.lineTo(p.x+Math.cos(a)*tip,p.y+Math.sin(a)*tip);ctx.strokeStyle=p.col+"ee";ctx.lineWidth=2;ctx.stroke();ctx.beginPath();ctx.arc(p.x+Math.cos(a)*tip,p.y+Math.sin(a)*tip,1.5,0,Math.PI*2);ctx.fillStyle=p.col+"cc";ctx.fill();}
  } else if(p.cat==="bacteria"){
    const angle=p.angle||Math.atan2(CY-p.y,CX-p.x);const hl=p.r*1.4,hw=p.r*0.72;
    ctx.save();ctx.translate(p.x,p.y);ctx.rotate(angle);
    ctx.beginPath();ctx.moveTo(-hl,-hw);ctx.lineTo(hl,-hw);ctx.arc(hl,0,hw,-Math.PI/2,Math.PI/2);ctx.lineTo(-hl,hw);ctx.arc(-hl,0,hw,Math.PI/2,-Math.PI/2);ctx.closePath();ctx.fillStyle=p.col;ctx.fill();ctx.strokeStyle=p.col+"99";ctx.lineWidth=1;ctx.stroke();
    const flCount=Math.max(1,Math.floor(p.r/7));
    for(let i=0;i<flCount;i++){const yOff=(i-(flCount-1)/2)*hw*0.9;const wag=now/600+i*1.4;ctx.beginPath();ctx.moveTo(-hl,yOff);ctx.bezierCurveTo(-hl-8,yOff+Math.sin(wag)*7,-hl-14,yOff+Math.sin(wag+1)*10,-hl-20,yOff+Math.sin(wag+2)*6);ctx.strokeStyle=p.col+"77";ctx.lineWidth=1.5;ctx.stroke();}
    ctx.restore();
  } else if(p.cat==="fungi"){
    ctx.beginPath();for(let i=0;i<=20;i++){const a=i/20*Math.PI*2;const lump=p.r*(1+0.28*Math.sin(i*2.1+now/1100+p.id)+0.14*Math.sin(i*3.7-now/800));i===0?ctx.moveTo(p.x+Math.cos(a)*lump,p.y+Math.sin(a)*lump):ctx.lineTo(p.x+Math.cos(a)*lump,p.y+Math.sin(a)*lump);}
    ctx.closePath();ctx.fillStyle=p.col;ctx.fill();ctx.strokeStyle=p.col+"88";ctx.lineWidth=1;ctx.stroke();
    const hyph=6;for(let i=0;i<hyph;i++){const a=i/hyph*Math.PI*2+now/2200;const base=p.r*(1+0.2*Math.sin(i*2.1+now/1100));ctx.beginPath();ctx.moveTo(p.x+Math.cos(a)*base,p.y+Math.sin(a)*base);ctx.lineTo(p.x+Math.cos(a)*(base+7),p.y+Math.sin(a)*(base+7));ctx.strokeStyle=p.col+"88";ctx.lineWidth=1.5;ctx.stroke();ctx.beginPath();ctx.arc(p.x+Math.cos(a)*(base+7),p.y+Math.sin(a)*(base+7),2,0,Math.PI*2);ctx.fillStyle=p.col+"99";ctx.fill();}
  } else if(p.cat==="parasite"){
    if(p.trail&&p.trail.length>0){for(let i=p.trail.length-1;i>=0;i--){const seg=p.trail[i];const frac=1-(i/p.trail.length);const sr=p.r*(0.45+frac*0.55);const alpha=Math.floor(frac*160).toString(16).padStart(2,"0");ctx.beginPath();ctx.arc(seg.x,seg.y,sr,0,Math.PI*2);ctx.fillStyle=p.col+alpha;ctx.fill();}}
    ctx.beginPath();ctx.arc(p.x,p.y,p.r,0,Math.PI*2);ctx.fillStyle=p.col;ctx.fill();ctx.strokeStyle=p.col+"cc";ctx.lineWidth=1.5;ctx.stroke();
    const eyeA=Math.atan2(CY-p.y,CX-p.x);const ex=p.x+Math.cos(eyeA)*p.r*0.35,ey=p.y+Math.sin(eyeA)*p.r*0.35;
...
export default function ImmunoWar(){
  const [chr,setChr]=useState(()=>{ try{ const s=localStorage.getItem("immunowar_player"); if(s) return JSON.parse(s); } catch(e){} return {gender:"female",age:"young",name:""}; });
  ...
  useEffect(()=>{try{localStorage.setItem("immunowar_player",JSON.stringify(chr));}catch(e){}},[chr]);
  ...
  const makeG=(mods,lvl,isBonus,comp,energy,adminRun=false)=>{const lev=isBonus?BONUS_LEVELS[lvl]:comp[lvl];return{cells:[],pathogens:[],projs:[],parts:[],bodyHp:mods.maxHp,maxHp:mods.maxHp,energy,eRegen:mods.eRegen,mods,lvl,wave:0,totalWaves:lev.waves,phase:"waveIdle",wideStart:null,spawnQ:[],score:0,countdown:4000,isBonus,compositions:comp,adminRun};};
  ...
  const spawnPath=(g,type,x,y,scale=1)=>{const pd=PATHS[type];g.pathogens.push({id:uid(),type,x,y,cat:pd.cat,hp:Math.round(pd.hp*scale),maxHp:Math.round(pd.hp*scale),spd:pd.spd*(1+(scale-1)*0.4),dmg:pd.dmg,rew:pd.rew,col:pd.col,r:pd.r,name:pd.name,splits:pd.splits||false,spawner:pd.spawner||false,spawnInt:pd.spawnInt||0,nextSpawn:0,wb:Math.random()*Math.PI*2,wbS:0.5+Math.random(),angle:0,trail:[],trailT:0});};
  ...
  const update=(dt,now)=>{
    ...
    for(const p of g.pathogens){
      ...
      const nx=dx/dist,ny=dy/dist;p.wb+=p.wbS*dt;
      if(p.cat==="virus"){p.x+=(nx*p.spd+(-ny)*Math.sin(p.wb)*0.28)*60*dt;p.y+=(ny*p.spd+nx*Math.sin(p.wb)*0.28)*60*dt;}
      else if(p.cat==="bacteria"){const wave=Math.sin(p.wb*2.2)*0.55;p.x+=(nx*p.spd+(-ny)*wave)*60*dt;p.y+=(ny*p.spd+nx*wave)*60*dt;p.angle=Math.atan2(dy,dx);}
      else if(p.cat==="fungi"){const ooze=Math.sin(p.wb*0.7)*0.18+Math.sin(p.wb*1.9)*0.08;p.x+=(nx*p.spd+(-ny)*ooze)*60*dt;p.y+=(ny*p.spd+nx*ooze)*60*dt;}
      else if(p.cat==="parasite"){const zig=Math.sin(p.wb*3.2)*0.85;p.x+=(nx*p.spd+(-ny)*zig)*60*dt;p.y+=(ny*p.spd+nx*zig)*60*dt;p.trailT++;if(p.trailT%2===0){p.trail.unshift({x:p.x,y:p.y});if(p.trail.length>8)p.trail.pop();}}
      else{p.x+=(nx*p.spd+(-ny)*Math.sin(p.wb)*0.28)*60*dt;p.y+=(ny*p.spd+nx*Math.sin(p.wb)*0.28)*60*dt;}
    ...
    for(const pj of g.projs){
      const tgt=g.pathogens.find(p=>p.id===pj.tid);if(tgt){pj.tx=tgt.x;pj.ty=tgt.y;}
      const dx=pj.tx-pj.x,dy=pj.ty-pj.y,d=Math.sqrt(dx*dx+dy*dy)||1;
      if(d<10){if(tgt&&tgt.hp>0){tgt.hp-=pj.dmg;addParts(g,tgt.x,tgt.y,pj.col,3);if(tgt.hp<=0){g.energy=Math.min(ECAP,g.energy+tgt.rew);g.score+=tgt.rew*10;addParts(g,tgt.x,tgt.y,tgt.col,14);SFX.play("kill");if(tgt.splits)died.push({x:tgt.x,y:tgt.y});}}pj.dead=true;}
  ...
  const draw=(ctx,now,selId)=>{
    ...
    for(let r=-1;r<CH/26+2;r++)for(let c=-1;c<CW/38+2;c++){const hx=c*38,hy=r*26+(c%2?13:0);ctx.beginPath();for(let i=0;i<6;i++){const a=i/6*Math.PI*2-Math.PI/6;i===0?ctx.moveTo(hx+Math.cos(a)*15,hy+Math.sin(a)*15):ctx.lineTo(hx+Math.cos(a)*15,hy+Math.sin(a)*15);}ctx.closePath();ctx.stroke();}
    const mx=mRef.current.x,my=mRef.current.y,sd=CELLS[selRef.current];
    if(mx>0&&mx<CW&&my>0&&my<CH&&!selId){ctx.beginPath();ctx.arc(mx,my,sd.r,0,Math.PI*2);ctx.fillStyle=sd.col+"40";ctx.fill();ctx.setLineDash([6,4]);ctx.beginPath();ctx.arc(mx,my,sd.range,0,Math.PI*2);ctx.strokeStyle=sd.col+"cc";ctx.lineWidth=1.5;ctx.stroke();ctx.setLineDash([]);}
    const pulse=(Math.sin(now/700)+1)/2;
    const cg=ctx.createRadialGradient(CX,CY,0,CX,CY,CR*4);cg.addColorStop(0,`rgba(0,180,255,${0.22+pulse*0.08})`);cg.addColorStop(0.6,"rgba(0,80,180,0.06)");cg.addColorStop(1,"transparent");
    ...
    const hr=g.bodyHp/g.maxHp;ctx.fillStyle=hr>0.5?"#fff":hr>0.25?"#FFD54F":"#EF5350";ctx.fillText(`${Math.ceil(g.bodyHp)}`,CX,CY);
    ...
    for(const c of g.cells){
      ...
      if(def.multishot){for(let mi=0;mi<3;mi++){const a=mi/3*Math.PI*2-Math.PI/2;ctx.beginPath();ctx.arc(c.x+Math.cos(a)*(def.r+6),c.y+Math.sin(a)*(def.r+6),2,0,Math.PI*2);ctx.fillStyle=def.col+"99";ctx.fill();}}
      ...
      if(c.id===selId){
        ...
        ctx.beginPath();ctx.arc(c.x,c.y,def.r+5+sp*2,0,Math.PI*2);ctx.strokeStyle=`rgba(255,255,255,${0.7+sp*0.3})`;ctx.lineWidth=2;ctx.stroke();
        ctx.font="7px monospace";ctx.textAlign="center";ctx.textBaseline="top";ctx.fillStyle="rgba(255,255,255,0.6)";ctx.fillText("DRAG TO MOVE",c.x,c.y+def.r+8);
      }
    }
  ...
  useEffect(()=>{
    ...
    const loop=now=>{
      ...
      if(!pausedRef.current){const dt=rawDt*gameSpeedRef.current;update(dt,now);}
      draw(ctx,now,selCellRef.current);
      const g=gRef.current;
      if(g){
        fRef.current++;
        if(fRef.current%4===0){const ld=getLev(g,g.lvl);setUi({hp:Math.ceil(g.bodyHp),maxHp:g.maxHp,energy:Math.floor(g.energy),cells:g.cells.length,lvl:g.lvl,wave:g.wave,totalWaves:g.totalWaves,score:g.score,phase:g.phase,countdown:Math.ceil(g.countdown/1000),isBonus:g.isBonus,levName:ld?.name||""});}
        if(g.phase==="gameOver"){if(!g.adminRun)saveScore({name:chr.name||"Anonymous",score:g.score,lvl:g.lvl+1,isBonus:g.isBonus,gender:chr.gender,age:chr.age,date:new Date().toLocaleDateString()}).then(setLb);setScreen("gameOver");scrRef.current="gameOver";return;}
        if(g.phase==="levelDone"){
          ...
          const ld=getLev(g,next);g.lvl=next;g.wave=0;g.totalWaves=ld.waves;g.phase="waveIdle";g.wideStart=null;g.pathogens=[];g.projs=[];
  ...
  const showMsg=(text,color="#EF9A9A")=>{setPlaceMsg({text,color});setTimeout(()=>setPlaceMsg(null),1600);};
  const toggleSfx=()=>{SFX.muted=!SFX.muted;setSfxOn(v=>!v);};
  const startBonus=()=>{const g=gRef.current;g.isBonus=true;g.lvl=0;g.wave=0;g.totalWaves=BONUS_LEVELS[0].waves;g.phase="waveIdle";g.wideStart=null;g.pathogens=[];g.projs=[];g.energy=Math.min(ECAP,g.energy+60);SFX.play("bonusStart");setBriefing({lvl:0,isBonus:true});scrRef.current="briefing";setScreen("briefing");};

  const sellCell=(cellId)=>{
    const g=gRef.current;if(!g)return;
    const id=cellId||selectedCell;if(!id)return;
    const idx=g.cells.findIndex(c=>c.id===id);if(idx<0)return;
    const c=g.cells[idx];const refund=Math.round(CELLS[c.type].cost*0.6);
    g.energy=Math.min(ECAP,g.energy+refund);g.cells.splice(idx,1);
    addParts(g,c.x,c.y,CELLS[c.type].col,12);
    setSelectedCell(null);showMsg(`+${refund}E energy refunded`,"#A5D6A7");
  };

  const getXY=(e)=>{const rect=cvsRef.current?.getBoundingClientRect();if(!rect)return{x:0,y:0};return{x:(e.clientX-rect.left)*(CW/rect.width),y:(e.clientY-rect.top)*(CH/rect.height)};};

  const handleMouseDown=e=>{
    const g=gRef.current;if(!g||scrRef.current!=="game")return;
    const{x,y}=getXY(e);
    for(const c of g.cells){
      if(Math.hypot(x-c.x,y-c.y)<CELLS[c.type].r+8){
        dragRef.current={cellId:c.id,origX:c.x,origY:c.y};
        setSelectedCell(c.id);e.preventDefault();return;
      }
    }
  ...
  const handleMouseMove=e=>{
    const{x,y}=getXY(e);mRef.current={x,y};
    const g=gRef.current;if(!g||!dragRef.current)return;
    const c=g.cells.find(c=>c.id===dragRef.current.cellId);if(c){c.x=x;c.y=y;}
  };
  ...
  const handleMouseUp=e=>{
    ...
    if(c){
      const def=CELLS[c.type];
      const bad=Math.hypot(c.x-CX,c.y-CY)<CR+def.r+8||g.cells.some(o=>o.id!==c.id&&Math.hypot(c.x-o.x,c.y-o.y)<def.r+CELLS[o.type].r)||c.x<def.r||c.x>CW-def.r||c.y<def.r||c.y>CH-def.r;
      if(bad){c.x=dragRef.current.origX;c.y=dragRef.current.origY;showMsg("Can't place there");}
      else SFX.play("place");
    }
  ...
  const handleClick=e=>{
    ...
    const{x,y}=getXY(e);
    for(const c of g.cells){if(Math.hypot(x-c.x,y-c.y)<CELLS[c.type].r+8){setSelectedCell(prev=>prev===c.id?null:c.id);return;}}
  ...
  const handleContextMenu=e=>{
    e.preventDefault();const g=gRef.current;if(!g||scrRef.current!=="game")return;
    const{x,y}=getXY(e);
    for(const c of g.cells){if(Math.hypot(x-c.x,y-c.y)<CELLS[c.type].r+8){sellCell(c.id);return;}}
  };
  ...
  if(showHowTo)return(
    <div style={{minHeight:"100vh",background:"#020c18",fontFamily:"monospace",color:"#e8f4f8",padding:"28px 20px",overflowY:"auto"}}>
      <div style={{maxWidth:620,margin:"0 auto"}}>
        ...
        <HSection title="OBJECTIVE" col="#80DEEA">Defend the glowing blue core at the center. Pathogens spawn from the edges and march toward it. If the core HP reaches zero the infection wins. Clear all waves in a level to advance. Beat all 10 levels to unlock the Bonus Cancer Round.</HSection>
        <HSection title="CONTROLS" col="#FFE082">
          <HRow label="Place cell" val="Click empty arena space" />
          <HRow label="Select/move cell" val="Click a cell, then drag it" />
          <HRow label="Sell a cell" val="Right-click it (60% energy back)" />
          <HRow label="Destroy (sidebar)" val="Click cell → DESTROY button" />
          <HRow label="Pause / Resume" val="Spacebar or ⏸ button in header" />
          <HRow label="Speed" val="Click 1× / 2× / 3× button in header" />
          <HRow label="Hotkeys" val="N · M · T · B to switch cell type" />
        </HSection>
        <HSection title="YOUR IMMUNE CELLS" col="#A5D6A7">
          {Object.entries(CELLS).map(([k,c])=>(
            <div key={k} style={{display:"flex",alignItems:"flex-start",gap:10,marginBottom:10}}>
              <div style={{width:12,height:12,borderRadius:"50%",background:c.col,flexShrink:0,marginTop:3}}/>
              <div><span style={{color:c.col,fontWeight:"bold",fontSize:12}}>{c.name}</span><span style={{color:"rgba(255,255,255,0.45)",fontSize:10}}> · {c.cost}E</span>
        ...
        <HSection title="ENEMY TYPES" col="#EF9A9A">
          {Object.entries(CAT_META).filter(([k])=>k!=="cancer").map(([k,v])=>(
            <div key={k} style={{display:"flex",gap:10,marginBottom:9}}>
              <div style={{width:12,height:12,borderRadius:"50%",background:v.col,flexShrink:0,marginTop:3}}/>
              <div><span style={{color:v.col,fontWeight:"bold",fontSize:12}}>{v.label}</span>
                <div style={{fontSize:10,color:"rgba(255,255,255,0.65)",marginTop:2}}>{k==="virus"?"Dart toward the core with a wobble. Spiky sphere shape.":k==="bacteria"?"Steady sinusoidal weave. Pill/rod shape with flagella.":k==="fungi"?"Slow ooze, irregular drift. Lumpy blob with hyphae.":"Aggressive S-curve zigzag. Segmented worm body."}</div>
              </div>
            </div>
          ))}
          <div style={{display:"flex",gap:10,marginTop:4}}>
            <div style={{width:12,height:12,borderRadius:"50%",background:"#8B0000",flexShrink:0,marginTop:3}}/>
            <div><span style={{color:"#EF5350",fontWeight:"bold",fontSize:12}}>CANCER (Bonus)</span>
              <div style={{fontSize:10,color:"rgba(255,255,255,0.65)",marginTop:2}}>Cells split on death. Tumors spawn reinforcements every 4.5 seconds.</div>
            </div>
          </div>
        </HSection>
        <HSection title="TIPS" col="#CE93D8">
          <div style={{fontSize:11,color:"rgba(255,255,255,0.7)",lineHeight:1.9}}>
            · Each run draws a different random enemy mix — every playthrough is a new biology lesson.<br/>
            · Neutrophils fire at 3 targets simultaneously — best against fast swarms.<br/>
            · T-Cells and B-Cells deal 25% more damage to their specialty enemies.<br/>
            · Sell a cell for 60% energy back and reposition mid-wave.<br/>
            · Set a checkpoint at any level — gives 2 respawn chances if you die.<br/>
            · Quick Retry on game over carries your score forward.
          </div>
        </HSection>
    ...
    <div style={{minHeight:"100vh",background:"#030c18",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"monospace",color:"#fff",position:"relative",overflow:"hidden",padding:24}}>
      ...
      <div style={{zIndex:1,textAlign:"center"}}>
        ...
        <div style={{display:"flex",gap:16,justifyContent:"center",marginBottom:36,flexWrap:"wrap"}}>
          {Object.entries(CAT_META).filter(([k])=>k!=="cancer").map(([k,v])=>(
            <div key={k} style={{display:"flex",alignItems:"center",gap:5,fontSize:9,color:v.col,letterSpacing:2}}><div style={{width:8,height:8,borderRadius:"50%",background:v.col}}/>{v.label}</div>
          ))}
        </div>
        ...
        {showLb&&<div style={{marginTop:16,maxWidth:480,width:"100%"}}><Leaderboard scores={lb} currentScore={null} currentName={null}/></div>}
        {adminMode&&(
  ...
  if(screen==="briefing"&&briefing!==null){
    const g=gRef.current;const levData=g?getLev(g,briefing.lvl):(briefing.isBonus?BONUS_LEVELS[briefing.lvl]:null);if(!levData)return null;
    const enemies=getBriefingEnemies(levData);const acCol=briefing.isBonus?"#FF6B6B":"#80DEEA";
    return(
      <div style={{minHeight:"100vh",background:briefing.isBonus?"#0d0004":"#020c18",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"monospace",color:"#e8f4f8",padding:"20px 16px"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10,maxWidth:560,width:"100%",marginBottom:16}}>
          {enemies.map(e=>{const cm=CAT_META[e.cat]||CAT_META.virus;return(
        ...
        {levData.tip&&<div style={{maxWidth:560,width:"100%",padding:"10px 15px",background:"rgba(255,220,100,0.06)",border:"1px solid rgba(255,220,100,0.2)",borderRadius:6,marginBottom:16,fontSize:11,color:"rgba(255,235,180,0.9)",lineHeight:1.6}}><span style={{color:"#FFE082",fontWeight:"bold"}}>STRATEGY: </span>{levData.tip}</div>}
        <button onClick={startPlay} style={BS(briefing.isBonus?"linear-gradient(135deg,#8B0000,#3D0040)":"linear-gradient(135deg,#00B0D8,#7B1FA2)")}>{briefing.lvl===0&&!briefing.isBonus?"DEPLOY NOW":"START"}</button>
        {briefing.lvl>0&&cpLeft>0&&(
          <div style={{marginTop:12,textAlign:"center"}}>
            {checkpoint?.lvl===briefing.lvl&&checkpoint?.isBonus===briefing.isBonus
              ...
              :<button onClick={()=>setCheckpoint({lvl:briefing.lvl,isBonus:briefing.isBonus})} style={{background:"transparent",border:"1px dashed rgba(255,220,100,0.4)",borderRadius:4,color:"rgba(255,220,100,0.7)",fontSize:10,letterSpacing:2,padding:"6px 16px",cursor:"pointer",fontFamily:"monospace"}}>{checkpoint?`MOVE CHECKPOINT HERE (currently L${checkpoint.lvl})`:"SET CHECKPOINT HERE (2 retries)"}</button>
            }
          </div>
        )}
      </div>
  ...
  if(screen==="game"){
    ...
    const sc=selectedCell?g?.cells.find(c=>c.id===selectedCell):null;
    return(
      <div style={{background:ui.isBonus?"#120005":"#020b16",height:"100vh",display:"flex",flexDirection:"column",fontFamily:"monospace",color:"#e8f4f8",overflow:"hidden"}}>
        ...
        <div style={{display:"flex",flex:1,overflow:"hidden"}}>
          <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",position:"relative",background:ui.isBonus?"#120005":"#030f1d"}}>
            <canvas ref={cvsRef} style={{display:"block",width:canvCss.w,height:canvCss.h,cursor:dragRef.current?"grabbing":selectedCell?"grab":"crosshair"}}
            ...
            {paused&&(
              <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,0.55)",pointerEvents:"none"}}>
                <div style={{textAlign:"center",fontFamily:"monospace"}}>
                  ...
                  <div style={{fontSize:10,color:"rgba(255,255,255,0.45)",marginTop:8,letterSpacing:2}}>SPACEBAR OR ▶ RESUME</div>
                </div>
              </div>
            )}
          ...
          <div style={{width:216,background:"rgba(0,0,0,0.58)",borderLeft:`1px solid ${ui.isBonus?"rgba(200,0,50,0.1)":"rgba(0,200,255,0.07)"}`,padding:12,display:"flex",flexDirection:"column",gap:10,overflowY:"auto",flexShrink:0}}>
            <div>
              ...
              <div style={{background:"rgba(255,255,255,0.1)",borderRadius:3,height:8}}><div style={{width:`${hpP*100}%`,height:"100%",borderRadius:3,transition:"width 0.2s",background:hpP>0.5?"#81C784":hpP>0.25?"#FFD54F":"#EF5350"}}/></div>
            ...
            <div>
              ...
              <div style={{background:"rgba(255,255,255,0.1)",borderRadius:3,height:8}}><div style={{width:`${eP*100}%`,height:"100%",borderRadius:3,background:"#FFD54F",transition:"width 0.1s"}}/></div>
              <div style={{fontSize:9,color:"rgba(255,255,255,0.65)",marginTop:1}}>{ui.energy}/{ECAP}</div>
            </div>

            {sc&&(
              <div style={{padding:"9px 10px",background:"rgba(255,255,255,0.06)",borderRadius:7,border:`2px solid ${CELLS[sc.type].col}55`}}>
                <div style={{fontSize:10,fontWeight:"bold",color:CELLS[sc.type].col,marginBottom:4}}>{CELLS[sc.type].name}</div>
                {CELLS[sc.type].catBonus&&<div style={{fontSize:8,color:"rgba(255,255,255,0.5)",marginBottom:4}}>{Object.entries(CELLS[sc.type].catBonus).map(([k,v])=>`+${Math.round((v-1)*100)}% vs ${k}`).join(" · ")}</div>}
                <div style={{fontSize:8,color:"rgba(255,255,255,0.4)",marginBottom:8}}>HP: {Math.ceil(sc.hp)}/{sc.maxHp} · Drag to reposition</div>
                <button onClick={()=>sellCell(sc.id)} style={{width:"100%",padding:"6px",fontSize:9,fontWeight:"bold",background:"rgba(239,83,80,0.15)",border:"1px solid rgba(239,83,80,0.35)",borderRadius:4,color:"#EF9A9A",cursor:"pointer",fontFamily:"monospace"}}>DESTROY (+{Math.round(CELLS[sc.type].cost*0.6)}E)</button>
            ...
            {ui.isBonus&&<div style={{padding:"7px 9px",background:"rgba(139,0,0,0.2)",borderRadius:5,border:"1px solid rgba(200,0,50,0.25)",fontSize:8,color:"rgba(255,170,170,0.85)",lineHeight:1.7}}>Cancer cells split on death. Tumors spawn reinforcements.</div>}
            <div style={{marginTop:"auto",fontSize:7,color:"rgba(255,255,255,0.35)",lineHeight:1.9}}>Click to place · Click cell to select<br/>Drag selected cell to move<br/>Right-click cell to sell<br/>Space = pause · N M T B = switch</div>
          </div>
        </div>
      </div>
    );
  ...
  // ─── GAME OVER ───────────────────────────────────────────────────────────────
  if(screen==="gameOver"){
    const g=gRef.current;const canRespawn=checkpoint&&cpLeft>0;
    return(
      <div style={{minHeight:"100vh",background:"#130208",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"monospace",color:"#e8f4f8",padding:20}}>
        ...
        <div style={{fontSize:18,color:"#FFE082",marginBottom:canRespawn?12:18}}>Score: {(g?.score??0).toLocaleString()}{g?.adminRun&&<span style={{fontSize:9,color:"rgba(255,150,50,0.7)",marginLeft:8}}>ADMIN — NOT SAVED</span>}</div>
        {canRespawn&&(
          <div style={{marginBottom:16,padding:"12px 20px",background:"rgba(255,220,100,0.07)",border:"1px solid rgba(255,220,100,0.25)",borderRadius:8,textAlign:"center"}}>
            ...
            <button onClick={respawnAtCheckpoint} style={{...BS("linear-gradient(135deg,#B8860B,#8B6914)"),padding:"9px 24px",fontSize:11}}>RESPAWN AT LEVEL {checkpoint.lvl+1}{checkpoint.isBonus?" (Cancer)":""}</button>
  ...
  if(screen==="victory"){const g=gRef.current;return(<div style={{minHeight:"100vh",background:"#011208",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"monospace",color:"#e8f4f8",padding:24}}><div style={{fontSize:10,letterSpacing:6,color:"#A5D6A7",marginBottom:8}}>ALL PATHOGENS ELIMINATED</div><h2 style={{fontSize:48,margin:"0 0 8px",color:"#81C784",letterSpacing:3}}>IMMUNITY</h2><div style={{fontSize:12,color:"rgba(255,255,255,0.6)",marginBottom:4}}>10 Levels Cleared.</div><div style={{fontSize:18,color:"#FFE082",marginBottom:22}}>Score: {(g?.score??0).toLocaleString()}</div><div style={{marginBottom:22,width:"100%",maxWidth:480}}><Leaderboard scores={lb} currentScore={g?.score} currentName={chr.name}/></div><button onClick={()=>{scrRef.current="title";setScreen("title");}} style={BS("linear-gradient(135deg,#00B0D8,#66BB6A)")}>PLAY AGAIN</button></div>);}

  if(screen==="bonusVictory"){const g=gRef.current;return(<div style={{minHeight:"100vh",background:"#04100a",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"monospace",color:"#e8f4f8",position:"relative",overflow:"hidden",padding:24}}><CancerVictoryArt/><div style={{zIndex:1,textAlign:"center",display:"flex",flexDirection:"column",alignItems:"center",maxWidth:520}}><div style={{fontSize:9,letterSpacing:6,color:"#81C784",marginBottom:14}}>BONUS STAGE COMPLETE</div><div style={{fontSize:50,fontWeight:900,letterSpacing:4,lineHeight:1.05,background:"linear-gradient(135deg,#A5D6A7,#FFE082,#80DEEA)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",marginBottom:10}}>THE BODY<br/>PREVAILED</div><div style={{fontSize:12,color:"rgba(255,255,255,0.65)",lineHeight:1.8,marginBottom:8}}>All cancer cells eliminated. All tumors destroyed.</div><div style={{fontSize:11,color:"rgba(200,230,200,0.55)",lineHeight:1.75,marginBottom:22,padding:"12px 18px",background:"rgba(129,199,132,0.05)",border:"1px solid rgba(129,199,132,0.12)",borderRadius:8}}>In real life, this battle happens in your body every day.<br/>The immune system destroys thousands of cancerous cells<br/>before they can form tumors — silently, without you knowing.</div><div style={{fontSize:18,color:"#FFE082",marginBottom:20}}>Final Score: {(g?.score??0).toLocaleString()}</div><div style={{marginBottom:22,width:"100%",maxWidth:480}}><Leaderboard scores={lb} currentScore={g?.score} currentName={chr.name}/></div><button onClick={()=>{scrRef.current="title";setScreen("title");}} style={BS("linear-gradient(135deg,#388E3C,#00B0D8)")}>PLAY AGAIN</button></div></div>);}

  return null;
}

// ─── HELPERS ─────────────────────────────────────────────────────────────────
function HSection({title,col,children}){return(<div style={{marginBottom:22}}><div style={{fontSize:9,letterSpacing:4,color:col,marginBottom:10,borderBottom:`1px solid ${col}33`,paddingBottom:6}}>{title}</div><div style={{fontSize:11,color:"rgba(255,255,255,0.7)",lineHeight:1.75}}>{children}</div></div>);}
function HRow({label,val}){return(<div style={{display:"flex",gap:12,marginBottom:5}}><span style={{color:"rgba(255,255,255,0.4)",minWidth:140,fontSize:10}}>{label}</span><span style={{color:"rgba(255,255,255,0.75)",fontSize:10}}>{val}</span></div>);}
...
function Leaderboard({scores,currentScore,currentName}){
  ...
  const medals=["🥇","🥈","🥉"];
  return(<div style={{width:"100%"}}><div style={{fontSize:9,letterSpacing:4,color:"rgba(255,255,255,0.4)",marginBottom:7,fontFamily:"monospace",textAlign:"center"}}>LEADERBOARD</div><div style={{display:"flex",flexDirection:"column",gap:4}}>{scores.map((s,i)=>{const isCur=currentScore&&s.score===currentScore&&s.name===(currentName||"Anonymous")&&i===scores.findIndex(x=>x.score===currentScore);return(<div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"5px 10px",background:isCur?"rgba(255,220,100,0.08)":"rgba(255,255,255,0.03)",borderRadius:5,border:`1px solid ${isCur?"rgba(255,220,100,0.25)":"rgba(255,255,255,0.07)"}`,fontFamily:"monospace"}}><span style={{fontSize:13,width:20,textAlign:"center"}}>{i<3?medals[i]:<span style={{fontSize:10,color:"rgba(255,255,255,0.35)"}}>{i+1}</span>}</span><span style={{fontSize:11,fontWeight:"bold",color:"#FFE082",minWidth:55}}>{s.score.toLocaleString()}</span><span style={{fontSize:10,fontWeight:"bold",color:isCur?"#FFE082":"rgba(255,255,255,0.75)",minWidth:70}}>{s.name||"Anonymous"}</span><span style={{fontSize:8,color:"rgba(255,255,255,0.45)",flex:1}}>Lvl {s.lvl}{s.bonusCompleted?" + Cancer":s.isBonus?" (bonus)":""}</span>{s.completed&&<span style={{fontSize:7,color:"#81C784",letterSpacing:1}}>CLEARED</span>}<span style={{fontSize:7,color:"rgba(255,255,255,0.25)"}}>{s.date}</span></div>);})}</div></div>);
}
...
function CancerVictoryArt(){
  ...
  useEffect(()=>{
    ...
    const strands=Array.from({length:6},(_,i)=>({phase:i*1.1,col:["#81C784","#FFE082","#80DEEA","#A5D6A7","#FFD54F","#4DB6AC"][i],spd:0.0005+i*0.0002}));
    let raf,t=0;
    const loop=()=>{t++;ctx.clearRect(0,0,cv.width,cv.height);
      for(const s of strands){ctx.beginPath();for(let x=0;x<cv.width;x+=4){const y=cv.height*0.5+Math.sin(x*0.008+s.phase+t*s.spd*60)*cv.height*0.22;x===0?ctx.moveTo(x,y):ctx.lineTo(x,y);}ctx.strokeStyle=s.col+"18";ctx.lineWidth=2;ctx.stroke();}
      for(const p of pts){p.x+=p.vx+Math.sin(t*0.02+p.ph)*p.wobble;p.y+=p.vy;if(p.y<-20){p.y=cv.height+10;p.x=Math.random()*cv.width;}const fade=Math.max(0,Math.min(1,(cv.height-p.y)/(cv.height*0.7)));ctx.beginPath();ctx.arc(p.x,p.y,p.r*fade,0,Math.PI*2);ctx.fillStyle=p.col+Math.floor(fade*180).toString(16).padStart(2,"0");ctx.fill();}
      const cx2=cv.width/2,ry=90,rs=28,pulse=(Math.sin(t*0.04)+1)/2,rc=`rgba(129,199,132,${0.55+pulse*0.2})`;
      ctx.strokeStyle=rc;ctx.lineWidth=4;ctx.lineCap="round";ctx.save();ctx.translate(cx2,ry);
      ...
      ctx.beginPath();ctx.moveTo(0,rs*0.6);ctx.bezierCurveTo(rs,rs*0.6,rs*1.4,-rs*0.5,rs*0.15,-rs*1.1);ctx.bezierCurveTo(0,-rs*1.6,-rs*0.15,-rs*1.1,0,-rs*0.4);ctx.stroke();
      ctx.beginPath();ctx.moveTo(0,rs*0.6);ctx.lineTo(-rs*0.5,rs*1.5);ctx.stroke();ctx.beginPath();ctx.moveTo(0,rs*0.6);ctx.lineTo(rs*0.5,rs*1.5);ctx.stroke();ctx.restore();
      raf=requestAnimationFrame(loop);};
    raf=requestAnimationFrame(loop);return()=>{cancelAnimationFrame(raf);window.removeEventListener("resize",resize);};
  },[]);
...
function FloatingBg(){
  ...
  useEffect(()=>{
    ...
    const resize=()=>{cv.width=window.innerWidth;cv.height=window.innerHeight;};
    resize();window.addEventListener("resize",resize);
    const cols=["#26C6DA","#66BB6A","#FFA726","#AB47BC","#EF5350","#8B4513","#FFB300"];
    ...
    let raf;
    const loop=t=>{ctx.clearRect(0,0,cv.width,cv.height);for(const it of its){it.x+=it.vx;it.y+=it.vy;if(it.x<-40)it.x=cv.width+40;if(it.x>cv.width+40)it.x=-40;if(it.y<-40)it.y=cv.height+40;if(it.y>cv.height+40)it.y=-40;const a=((Math.sin(t/1300+it.ph)+1)/2)*0.12+0.03;const g2=ctx.createRadialGradient(it.x,it.y,0,it.x,it.y,it.r*1.5);g2.addColorStop(0,it.col+Math.floor(a*255).toString(16).padStart(2,"0"));g2.addColorStop(1,"transparent");ctx.fillStyle=g2;ctx.beginPath();ctx.arc(it.x,it.y,it.r*1.5,0,Math.PI*2);ctx.fill();}raf=requestAnimationFrame(loop);};
    raf=requestAnimationFrame(loop);return()=>{cancelAnimationFrame(raf);window.removeEventListener("resize",resize);};
  },[]);
